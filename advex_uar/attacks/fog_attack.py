...
class FogAttack(AttackWrapper):
    ...
    def _forward(self, pixel_model, pixel_img, target, avoid_target=True, scale_eps=False):
        if pixel_img.shape[2] == 28:
            pixel_img = torch.nn.functional.pad(pixel_img,(2,2,2,2))
            pixel_img = pixel_img.repeat(1,3,1,1)
            mnist = True
            self.resol = 32
        else:
            mnist = False
        ...
        pixel_result = torch.clamp((pixel_inp + base_eps[:, None, None, None] * fog) /
                                   (x_max + base_eps[:, None, None, None]) * 255., 0., 255.)
        if mnist:
            pixel_result = pixel_result[:,:,2:30,2:30]
            pixel_result = torch.mean(pixel_result,1,keepdim=True)
        return pixel_result
